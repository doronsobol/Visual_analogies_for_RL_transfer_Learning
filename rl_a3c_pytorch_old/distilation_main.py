...
from utils import read_config, get_num_of_actions
...
if __name__ == '__main__':
    ...
    shared_model = A3Clstm(env.observation_space.shape[0], get_num_of_actions(args))
    if args.load:
        saved_state = torch.load(
            '{0}'.format(args.load_model_file))
        current_state = shared_model.state_dict()
        current_state.update(saved_state)
        shared_model.load_state_dict(current_state)
