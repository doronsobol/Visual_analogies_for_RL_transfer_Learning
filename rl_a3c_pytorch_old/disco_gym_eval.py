...
parser.add_argument('--random',default=False,help="if to use random sampling")
parser.add_argument(
    '--model_env',
    default='Pong-v0',
    metavar='MENV',
    ...
    for i_episode in tqdm(range(args.num_episodes)):
        ...
        for _ in tqdm(iter(int, 1)):
            ...
            if args.keep_images:
                ...
                print('{0}/{1:07d}.jpg: {2}'.format(args.images_dir,im_num ,":".join([" ".join([str(i) for i in c.data.numpy().flatten()]) for c in convs[-2:]])), file=c_labels_file)
            ...
            if second:
                second = False
                corrected_action = 1
            if args.random or (args.use_orig_model and np.random.choice([False, False, False, True])):
