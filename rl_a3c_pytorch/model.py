...
class shared_A3Clstm(torch.nn.Module):
    def __init__(self, num_inputs, action_space):
        super(shared_A3Clstm, self).__init__()

        self.lstm = nn.LSTMCell(3, 512)
        num_outputs = action_space+2
        self.critic_linear = nn.Linear(512, 1)
        self.actor_linear = nn.Linear(512, num_outputs)
        self.conv1 = nn.Conv2d(256, 128, 4, stride=1, padding=1)
...
class A3Clstm_embeddings(torch.nn.Module):
    def __init__(self, action_space):
        super(A3Clstm_embeddings, self).__init__()
        ...
        self.actor_linear.weight.data = norm_col_init(
            self.actor_linear.weight.data, 0.01)
        self.actor_linear.bias.data.fill_(0)
        #self.embeddings.weight.data = norm_col_init(
    ...
    def forward(self, inputs):
        ...
        return self.critic_linear(x), self.actor_linear(x), (hx, cx), x, x
